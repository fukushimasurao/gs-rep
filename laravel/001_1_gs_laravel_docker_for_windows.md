...
{% hint style="warning" %}
**プロジェクト作成コマンドについて**
動画では `curl -s "https://laravel.build/laratter" | bash` でプロジェクトを作成していますが、現在はLivewire Starter Kit + Fortify構成に対応するため、下記テキストの新しいコマンドを使用してください。Docker Desktop・WSLのセットアップ手順自体は動画の通りで問題ありません。
{% endhint %}
