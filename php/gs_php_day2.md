...
| コマ3（50分） | セキュリティ体感・まとめ | SQLインジェクション体感 → 宿題説明 |
...
| | varchar | text |
|---|---|---|
| 向いている文字数 | 〜255文字程度 | 256文字〜 |
| 使用例 | 名前・メール・タイトル | 本文・コメント・説明文 |
| パフォーマンス | 短いデータは高速 | 長いデータ向き |

**判断の目安：「上限が決まっているか？」**

* 上限が決まっている → `varchar`（例：名前は64文字以内、メールは128文字以内）
* 上限が不明・長くなりうる → `text`（例：お問い合わせ本文、ブログ記事）

今回のテーブルも `name: varchar(64)`、`content: text` とこの考え方で使い分けています。
{% endhint %}
...
本編でXSS攻撃を体験した。今度は`h()`が正しく防いでいるか自分で確認してみよう。

1. `<script>alert('XSS')</script>` をフォームから投稿
2. select.phpを開いてアラートが**出ない**ことを確認
3. `select.php`の`h()`を一時的に外して同じデータを投稿
4. アラートが**出る**ことを確認（攻撃成功）
5. `h()`を戻して防げることを再確認
