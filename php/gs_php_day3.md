...
{% hint style="info" %}
**require, require_once, include, include_once の違い**

* `require`：ファイルが読み込めない場合、致命的なエラーで処理が止まる
* `include`：ファイルが読み込めない場合、警告（warning）が出るだけで処理は続行される
* `require_once`：`require`と同じだが、同じファイルを2回以上読み込まないようにする
* `include_once`：`include`と同じだが、同じファイルを2回以上読み込まないようにする

**おすすめ**：`funcs.php`のような「絶対に必要で、1回だけ読み込めば十分」な共通ファイルには`require_once`を使うのが基本です。読み込みに失敗したら処理を止めたい（`require`の特性）かつ、複数のファイルからrequireされても二重読み込みされない（`_once`の特性）という、両方の利点を兼ね備えているためです。

参考：[require, require\_once, include, include\_once の違い](https://qiita.com/awesam86/items/3fa28e23c95ca74caddc)
{% endhint %}
...
今のdelete.phpは、ボタンを押した瞬間に即削除されます。実際のWebサービスでは「本当に削除しますか？」という確認画面を挟むことが多いです。AIに相談しながら実装してみましょう。

【サンプルプロンプト】
```
【コンテキスト】
- PHPの授業でdelete.phpを作りました。今はボタンを押すと即座に削除される仕組みです
- DBはMySQLで、接続にはPDOを使います

【依頼】
削除ボタンを押したら、いきなり削除するのではなく
「本当に削除しますか？」という確認画面を1枚挟みたいです。
どんな実装方法がありますか？初学者向けにいくつか案を教えてください。
```

出てきた案を実際に試してみて、どの方法が今の授業の構造（4ブロック構造）に一番合っているか考えてみましょう。
...
### ④ 関数について理解を深める（作成方法・引数・戻り値）

コマ3で`db_conn()`・`sql_error()`・`redirect()`という3つの関数を作りました。ここで改めて「関数とは何か」をAIに聞いて整理してみましょう。

【サンプルプロンプト】
```
PHPの関数について、初学者向けに教えてください。

【質問】
1. 関数はどうやって作りますか？基本の書き方を教えてください
2. 「引数」とは何ですか？何のためにありますか？
3. 「戻り値（return）」とは何ですか？何のためにありますか？
...
AIの説明を読んだら、`sql_error($stmt)`と`redirect($file_name)`を例に、以下を自分の言葉で説明できるか確認してみましょう。

* `$stmt`や`$file_name`が「引数」であること
* `db_conn()`だけが`return $pdo;`で「戻り値」を返していて、`sql_error()`や`redirect()`は返していないこと（`exit()`で処理が終わるため）

**発展：自分で新しい関数を作ってみる**

funcs.phpに、`h()`のような「引数を受け取り、戻り値を返す」関数を自分でもう1つ作ってみましょう。例えば、

```
PHPの授業で、年齢を受け取って「20歳未満」「20歳以上」を文字列で返す関数を
作りたいです。関数名の案と実装をいくつか教えてください。
```

のようにAIに相談し、実際にfuncs.phpに追加してselect.phpなどから呼び出してみてください。

### ⑤ バリデーション（受け取った内容のチェック）を入れてみる

今の`insert.php`や`update.php`は、`$_POST`で受け取った内容をチェックせずそのままSQLに使っています。実際のWebサービスでは「空で送られてきていないか」「ageが数字になっているか」など、受け取った内容を精査してからDBに保存するのが大切です。AIに相談しながら実装してみましょう。

【サンプルプロンプト】
```
【コンテキスト】
- PHPの授業でinsert.php（登録処理）を作りました
- $_POST['name']、$_POST['email']、$_POST['age']、$_POST['content']を
  そのままSQLの実行に使っています

【依頼】
DBに保存する前に、受け取った内容が正しいかチェック（バリデーション）を
入れたいです。
- 未入力の項目がないか
- ageが数字になっているか
などを確認する場合、どんな実装方法がありますか？初学者向けにいくつか案を教えてください。
```

出てきた案を試してみて、チェックに引っかかった場合にどう処理する（エラーメッセージを出す・登録処理を止める等）のが良いか、今の4ブロック構造に合わせて考えてみましょう。
