...
{% hint style="warning" %}
フォームから受け取った値をそのまま出力することはセキュリティ上危険です。対策は次回扱います。
{% endhint %}
...
具体的に、`post_confirm.php`を以下のように変更してみてください。
...
{% hint style="info" %}
`PHP_EOL` は環境に応じた適切な改行コードを表す定数です（Windows: `\r\n` / Mac・Linux: `\n`）。`"\n"` の代わりに使うことで、どちらの環境でも正しく改行されます。
{% endhint %}
...
* `action`: 送信先が `write.php` になっているか
* `method`: `post` になっているか（`write.php` 側で `$_POST` で受け取るため）
